...
from database_module_balanced_50 import database_module
import copy
    ...
    def __init__(self,
        ...
        with tf.name_scope("accuracy"):
            with tf.name_scope('accuracy_species'):
                ...
                self.species_matched_list = tf.equal(prediction,tf.argmax(y_onehot1,1))
        ...
        with tf.name_scope("train"):
            ...
            matched_predictions = tf.Variable(tf.zeros_like(self.species_matched_list), trainable=False)
            ...
            self.get_matched_predictions = tf.assign(matched_predictions, self.species_matched_list)
                ...
                try:
                    ...
                    if i % self.val_freq == 0:                        
                        ...
                        for j in range(self.val_iter):
                            img,lbl1,lbl2,lbl3,lbl4,lbl5,filepaths = self.test_database.read_batch()
                            sess.run(
                    ...
                    for j in range(self.iterbatch):
                        img,lbl1,lbl2,lbl3,lbl4,lbl5,filepaths = self.train_database.read_batch()
                        
                        
                        self.speciesPredictionList = sess.run(self.get_matched_predictions,
                                                            feed_dict = {
                                                                    x: img,
                                                                    y1 : lbl1,
                                                                    y2 : lbl2,
                        ...
                        for fp, acc in zip(filepaths, self.speciesPredictionList):
                            if acc == False:                                        
                                current_wrong_predictions.append(fp)
                    ...
                    current_epoch = copy.deepcopy(self.train_database.epoch)
                    
                    if current_epoch > prev_epoch:    

                        misclassified_txt = "epoch_" + str(current_epoch) + ".txt"
                        misclassified_txt_path = os.path.join(self.save_dir, misclassified_txt)
                        
                        current_wrong_predictions = list(set(current_wrong_predictions))
                        
                        print("Writing misclassified samples:", str(len(current_wrong_predictions)))
                        with open(misclassified_txt_path, 'a') as t1:
                            for fp in current_wrong_predictions:
                                t1.write("%s\n" % (fp))  
                        
                        self.train_database.update_negative_list(current_wrong_predictions)
                        current_wrong_predictions = []
